...
if __name__ == '__main__':
    rospy.init_node('z_client')

    client = ModbusTcpClient('192.168.11.4', 502)
    connection = client.connect()
    if (connection):
        rospy.loginfo('Connected to Z motor on 192.168.11.4:502')

        configFile = open('src/deman_motor_clients/config.json')
        configData = json.load(configFile)

        config = configData['z_client']
        ...
        subPosition = rospy.Subscriber('/z_client/position', Int32, callback=position_callback)
        pubStatus = rospy.Publisher('/z_client/status', String, queue_size=10)
        pubActualPosition = rospy.Publisher('/z_client/actual_position', Int32, queue_size=10)
        rate = rospy.Rate(20)
        ...
        rospy.loginfo('Failed to connect to Z motor')
        rospy.signal_shutdown('Failed to connect to Z motor')
